...
export abstract class Borg {
  ...
  abstract try(input: unknown): B.TryResult<this>;
...
> extends Borg {
  ...
  try(input: unknown): B.TryResult<this> {
    try {
...
> extends Borg {
  ...
  get meta(): Meta.NumberMeta<TFlags, TLength> {
    return Object.freeze({
      kind: "number",
      max: this.#max,
      min: this.#min,
  ...
  min, remove min.
  */
  min<const N extends number | null>(min: N): BorgNumber<TFlags, [N, TLength[1]]> {
    const clone = this.copy();
    clone.#min = min;
    return clone as any;
  }

  max<const N extends number | null>(max: N): BorgNumber<TFlags, [TLength[0], N]> {
    const clone = this.copy();
    clone.#max = max;
    return clone as any;
  }
   ...
   * Inclusive range
   */
  range<const N extends number | null, const M extends number | null>(
    min: N,
    max: M,
...
>
  ...
  : TBorg extends BorgArray<infer TItemBorg, infer TFlags, infer TLength>
  ? Meta.ArrayMeta<TItemBorg, TFlags, TLength>
  : TBorg extends BorgString<infer TFlags, infer TLength, infer TPattern>
  ? Meta.StringMeta<TFlags, TLength, TPattern>
  : TBorg extends BorgNumber<infer TFlags, infer TRange>
  ? Meta.NumberMeta<TFlags, TRange>
  : TBorg extends BorgBoolean<infer TFlags>
